...
def index(request):
	...
	if username:
		return render_to_response('index.html', {'username':username,'products':products})
	else:
...
def search(request):
	...
	if request.method == 'POST':
		...
		return render_to_response('search_result.html', {'username':username, 'product_name':product_name,
			'search_result':search_result, 'search_status':search_status, 'result_len':result_len,'products':products})
	else:
		return HttpResponse("!!!")
